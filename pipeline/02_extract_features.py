...
def compute_raster_features(mg, mask):
    ...
    Note: Landscape width Ly is stored in le_params (Stage 1) and flows
    into the feature row via dict merging in Stage 2. It is not computed
...
# Mapping from legacy le_params key names to refactored names.
# Legacy rasnet files (generated before refactoring) used different column
# names for LE parameters and identifiers.
_LEGACY_LE_PARAM_RENAMES = {
    'tmux-sess': 'job_id',
    'df-ind':    'landscape_idx',
    'ts-ind':    'ts_index',
    'ly':        'Ly',
}


def _convert_legacy_rasnet_params(le_params: dict) -> dict:
    """Rename legacy le_params keys to refactored conventions.

    Legacy rasnet files (generated before pipeline refactoring) stored
    LE parameters and identifiers under different key names. This function
    detects and renames any legacy keys so that Stage 2 feature extraction
    produces consistent column names regardless of whether the rasnet file
    was generated by the legacy or refactored code.
    ...
    ----------
    le_params : dict
        As loaded from a rasnet pickle file.

    Returns
    -------
    dict
        le_params with any legacy keys renamed to refactored conventions.
    """
    for old, new in _LEGACY_LE_PARAM_RENAMES.items():
        if old in le_params:
            le_params[new] = le_params.pop(old)
    return le_params
...
def run_stage2_features(data_dir, output_dir, job_id, git_hash='latest'):
    ...
    for i, rasnet_file in enumerate(rasnet_files):
        ...
        le_params = _convert_legacy_rasnet_params(le_params)
